<?php
/*
Plugin Name: IDX Broker Original
Plugin URI: http://www.idxbroker.com/support/kb/categories/Wordpress+Plugin/
Description: A premium IDX WordPress plugin. The IDX Broker Original plugin gives Realtors&reg; an easier way to add IDX Broker Widgets, Menu links, and Custom Links to any WordPress website. This plugin is designed exclusively for IDX Broker subscribers.
Version: 1.5.3
Author: IDX, LLC
...
//Adds a comment declaring the version of the IDX Broker Original Plugin if it is activated.
function idxBrokerActivated() {
echo "\n<!-- IDX Broker Original WordPress Plugin v1.5.3 Activated -->\n\n";
}
...
function idx_broker_menu() {
  ...
  add_options_page('IDX Broker Original Plugin Options', 'IDX Broker', 'administrator', 'idx-broker', 'idx_broker_admin_page');
...
<div id="idxPluginWrap" class="wrap"> <br class="clear" />
  ...
  <h2 style="float: left;">IDX Broker&reg; Original Plugin Options</h2>
  <br class="clear" />
  ...
  */
  ...
  if (empty($cid) || !is_numeric($cid)) {
    ...
    */
    ...
    $errors[] = "Please check your Client Identification (CID) in the IDX Broker Original Plugin Settings for errors.";
  ...
  */
  ...
  if (empty($domain)) {
    ...
    */
    ...
    $errors[] = "Please enter the full domain of IDX hosted pages in the IDX Broker Original Plugin Settings.";
  }
    else { return false;
    }
...
<?php
  ...
  function form($instance) {
    ...
    echo '<div id="idxLeadLogin-admin-panel">';
    echo '<p>This will add any custom link pages that you have added using your IDX Broker Original Plugin.</a></p>';
    echo '<label for="' . $this->get_field_id("title") .'">Widget Title:</label>';
    echo '<input type="text" ';
    echo 'name="' . $this->get_field_name("title") . '" ';
    echo 'id="' . $this->get_field_id("title") . '" ';
